...
def generateUsersItems(obsFnamePath, unobsFnamePath) :
...
def generateNewRating(obsFnamePath, unobsFnamePath) :
    obsFile = open(obsFnamePath, "r")
    unobsFile = open(unobsFnamePath, "r")
...
def generateTargetRating(unobsFnamePath) :
    #obsFile = open(obsFnamePath, "r")
    unobsFile = open(unobsFnamePath, "r")
...
#generateUsersItems(filename_obs, filename_uno)
#generateNewRating(filename_obs, filename_uno)


# generate user_item matrix for rating matrix
def generateRatingMatrix(obsFnamePath):
    cwd = os.getcwd()
    parent_cwd = os.path.dirname(cwd)
    data_dir = parent_cwd + "/data/simple-recommender/0/eval"
...
generateRatingMatrix(filename_obs)

#generateTargetRating(filename_uno)
