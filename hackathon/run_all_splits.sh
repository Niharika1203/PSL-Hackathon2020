...
function run_psl() {
    ...
    pushd . > /dev/null
        ...
        # Run PSL.
        gtime -v --output="${timePath}" ./run.sh ${extraOptions} > "${outPath}" 2> "${errPath}"
...
function run_example() {
    ...
    for splitId in $(ls -1 "${exampleDir}/data/${exampleName}") ; do
        ...
        gsed -i "s#data/${exampleName}/[0-9]\\+#data/${exampleName}/${splitId}#g" "${cliDir}/${exampleName}"*.data
    ...
    gsed -i "s#data/${exampleName}/[0-9]\\+#data/${exampleName}/0#g" "${cliDir}/${exampleName}"*.data
}
